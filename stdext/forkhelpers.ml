...
let execute_command_get_output_inner ?env ?stdin cmd args =
	...
	Pervasiveext.finally (fun () -> 
		match with_logfile_fd "execute_command_get_out" (fun out_fd ->
			with_logfile_fd "execute_command_get_err" (fun err_fd ->
				let (sock,pid) = safe_close_and_exec ?env (Opt.map (fun (_,fd,_) -> fd) stdinandpipes) (Some out_fd) (Some err_fd) [] cmd args in
...
let execute_command_get_output ?env cmd args =
	execute_command_get_output_inner ?env ?stdin:None cmd args

let execute_command_get_output_send_stdin ?env cmd args stdin =
	execute_command_get_output_inner ?env ~stdin cmd args
